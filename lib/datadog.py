...
def _get_service():
    ...
    if dd_service is None:
        dd_service = buildpackutil.get_appname()
    return dd_service
...
def _set_up_postgres():
    ...
    os.makedirs(DD_AGENT_CHECKS_DIR + "/postgres", exist_ok=True)
    with open(DD_AGENT_CHECKS_DIR + "/postgres/conf.yaml", "w") as fh:
        config = {
            "init_config": {},
            "instances": [
                {
